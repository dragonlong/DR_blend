...
from classify_loader_dynamic_sampling_3ch_aug_MSEtune_gpu2 import data_loader
from inception_v3 import inception_v3 as icp_v3_new
from densenet_custom import DenseNet3
from densenet2 import DenseNet
from DR_RAM_512 import c_512_5_3_32 as ram
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_loader = torch.utils.data.DataLoader(
            data_loader('/data/jeffery/kg/sample_512/', True, 0, transform=
            transforms.Compose([
                transforms.ToTensor(),
                # transforms.Normalize((-257.478639,), (471.683592,)),
            ])
        ...
        test_loader = torch.utils.data.DataLoader(
            data_loader('/data/jeffery/kg/sample_512/', False, epoch, transform=
            transforms.Compose([
                transforms.ToTensor(),
                # transforms.Normalize((-257.478639,), (471.683592,)),
            ])
...
def validate(val_loader, model, criterion):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        acc = accuracy(output_one_hot.data, target_var.data)
        # measure accuracy and record loss
        ...
        '''
        acc_0, num_0 = accu_for_cls(output_one_hot.data, target_var.data, 0)
        acc_1, num_1 = accu_for_cls(output_one_hot.data, target_var.data, 1)
        acc_2, num_2 = accu_for_cls(output_one_hot.data, target_var.data, 2)
        acc_3, num_3 = accu_for_cls(output_one_hot.data, target_var.data, 3)
        acc_4, num_4 = accu_for_cls(output_one_hot.data, target_var.data, 4)
        ...
        cfm.add(output_one_hot, target_var)
        # measure elapsed time
        batch_time.update(time.time() - end)
        end = time.time()
...
def to_one_hot(predict):
    predict_np = predict.cpu().data.numpy()
    res = []
    for i in range(len(predict_np)):
        idx = np.argmin(np.array(
            [np.abs(predict_np[i] - 0), np.abs(predict_np[i] - 1), np.abs(predict_np[i] - 2), np.abs(predict_np[i] - 3),
             np.abs(predict_np[i] - 4)]), axis=0)
        one_hot = np.zeros(5)
        one_hot[idx] = 1
        res.append(one_hot)
    return torch.autograd.Variable(torch.from_numpy(np.array(res))).cuda()
...
def accuracy(output, target):
    ...
    correct = o.eq(target.type(torch.LongTensor).cuda())
    correct = correct.sum()
