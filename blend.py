...
class BlendNet(nn.Net):
    ...
    def set_split(self, files, labels):
        ...
        def split(X, y, eval_size):
            if eval_size:
                tr, te = data_orig.split_indices(files, labels, eval_size)
...
class ResampleIterator(BatchIterator):
    ...
    def __iter__(self):
        ...
        bs = self.batch_size
        indices = data_orig.balance_per_class_indices(self.y.ravel())
...
image_files = data_orig.get_image_files(config.get('train_dir'))
names = data_orig.get_names(image_files)
labels = data_orig.get_labels(names).astype(np.float32)[:, np.newaxis]

if features_file is not None:
    runs = {'run': [features_file]}
else:
    runs = data_orig.parse_blend_config(yaml.load(open(blend_cnf)))

scalers = {run: StandardScaler() for run in runs}

tr, te = data_orig.split_indices(image_files, labels)
...
for i in range(n_iter):
    ...
    for run, files in runs.items():
        ...
        X = data_orig.load_features(files)
        X = scalers[run].fit_transform(X)
        X = data_orig.per_patient_reshape(X) if per_patient else X
        ...
        if not predict:
            ...
            print("kappa after run {}, iter {}: {}".format(
                run, i, utils.kappa(labels[te], y_pred)))
            print("confusion matrix")
            print(confusion_matrix(labels[te], y_pred))
        else:
            X = data_orig.load_features(files, test=True)
            X = np.nan_to_num(X)
...
if predict:
    ...
    submission_filename = utils.get_submission_filename()
    image_files = data_orig.get_image_files(test_dir or config.get('test_dir'))
    names = data_orig.get_names(image_files)
